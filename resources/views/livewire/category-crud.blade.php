                        ...
                        @forelse ($categories as $category)
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-2 flex space-x-2">
                                    <button wire:click="edit({{ $category->id }})" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md">Edit</button>
                                    <button wire:click="delete({{ $category->id }})" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md">Delete</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="text-center px-4 py-2">No categories found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="relative w-full max-w-2xl bg-white rounded-lg shadow-lg p-6">
                <div class="relative bg-white rounded-lg shadow">
                    <div class="flex items-center justify-between p-4 border-b">
                        <h5 class="text-xl font-semibold">{{ $categoryId ? 'Edit Category' : 'Create Category' }}</h5>
                    ...
                    <div class="flex items-center p-4 border-t">
                        <button wire:click="createOrUpdateCategory()" class="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded-lg">
