            ...
            <div class="card-body p-4">
                ...
                <div class="mb-4">
                    ...
                    @elseif ($product->image)
                        <img src="{{ Storage::url($product->image) }}" class="mt-2 w-48 h-48 object-cover rounded-md" />
                    @endif
                </div>

                <!-- Description (CKEditor Integration) -->
                <div class="mb-4 prose" wire:ignore x-data x-init="
    ClassicEditor.create($refs.editor)
    .then(editor => {
        editor.model.document.on('change:data', () => {
            @this.set('description', editor.getData()); // Make sure editor.getData() returns the HTML
        });
    })
    .catch(error => {
        console.error(error);
...
">
    ...
    <textarea x-ref="editor" wire:model.defer="description" class="form-control w-full p-2 border rounded-md prose" rows="4">{{ $product->description }}</textarea>
    @error('description') <span class="text-red-500">{{ $message }}</span> @enderror
                ...
                <!-- Update Button -->
                <div class="mb-4">
                    <button class="btn btn-primary bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Update</button>
                </div>
            </div>
        </div>
    </form>
